...
export async function getUserId(userName: string): Promise<string> {
  return await axiosHelper.get(IrisUrl.IRIS_API_GET_AUTH_GET_USER_ID, { params: { userName } });
}
...
export async function challenge(userId: string): Promise<ChallengeResponse> {
  return await axiosHelper.get<ChallengeResponse>(IrisUrl.IRIS_API_GET_AUTH_CHALLENGE, {
    params: { userId },
  });
}
...
export async function loginRequest(userId: string, passwordHash: string): Promise<LoginResponse> {
  return await axiosHelper.post<LoginResponse>(IrisUrl.IRIS_API_POST_AUTH_LOGIN, {
    userId,
    passwordHash,
  });
}
...
export async function logoutRequest(): Promise<LogoutResponse> {
  return await axiosHelper.post<LogoutResponse>(IrisUrl.IRIS_API_POST_LOGOUT);
}
