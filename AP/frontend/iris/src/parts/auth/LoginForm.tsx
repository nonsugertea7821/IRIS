...
export default function LoginForm(): JSX.Element {
  ...
  return (
    <AppContainer>
      <Box sx={useMemo(() => ({ display: 'flex', flexDirection: 'column', gap: 2, margin: 10 }), [])}>
        <Box sx={useMemo(() => ({ display: 'flex', flexDirection: 'row', gap: 1 }), [])}>
          <TextField
            type='text'
            label='Host'
            value={host}
        ...
        <TextField type='text' label='username' value={userName} onChange={handleUserNameChange} disabled={loading} />
        <TextField
          type='password'
          label='password'
          value={password}
